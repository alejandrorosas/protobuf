...
TEST(GeneratedCode, MessageUnknown_Encode_StringView) {
  upb_Arena* arena = upb_Arena_New();
  upb_MessageUnknown unknown;
  unknown.type = kUpb_MessageUnknownType_StringView;
  unknown.value.bytes = upb_StringView_FromString("Hello");

  upb_StringView view;
  upb_EncodeStatus status = upb_MessageUnknown_Encode(&unknown, arena, &view);
  EXPECT_EQ(kUpb_EncodeStatus_Ok, status);
  EXPECT_EQ(unknown.value.bytes.data, view.data);
  EXPECT_EQ(unknown.value.bytes.size, view.size);
...
TEST(GeneratedCode, MessageUnknown_Encode_NonCanonicalExtension) {
  ...
  upb_EncodeStatus status =
      upb_MessageUnknown_Encode(&result.unknown, arena, &view);
  EXPECT_EQ(kUpb_EncodeStatus_Ok, status);
