...
// Encodes a `upb_MessageUnknown` to bytes.
//
// If it is already a `upb_StringView`, it is returned directly; otherwise, it's
// a non-canonical extension that is serialized. Returns `kUpb_EncodeStatus_Ok`
// on success.
UPB_NODISCARD upb_EncodeStatus
upb_MessageUnknown_Encode(const struct upb_MessageUnknown* unknown,
                          struct upb_Arena* arena, upb_StringView* view);
